...
bond_returns=excess_return['average_excess_return'].to_frame()
...
def log_forward_rate(b2,b1):
    b2=b2.join(b1['log_price'].rename('log_price_pre'))
    b2['log_forward_rate']=b2.apply(lambda x: x['log_price_pre']-x['log_price'],axis=1)
    return b2['log_forward_rate']
